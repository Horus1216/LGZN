...
#pragma mark 添加表情、+按钮
- (void)addIcons{
    _icons=@[self.faceBtn/*,self.otherBtn*/];//update by qzlei
...
//        [_chatInputTextView setFrame:CGRectMake(CGRectGetMaxX(self.otherBtn.frame)+3, 5,kScreenWidth-150, hight_text_one+20)];
        [_chatInputTextView setFrame:CGRectMake(CGRectGetMaxX(self.faceBtn.frame)+3, 5,kScreenWidth-120, hight_text_one+20)];//update by qzlei
...
-(void)sendMessage{
    ...
    if (plainText.length == 0) {
        NSLog(@"ChatKeyBoardView:发送的内容不能为空!!");
    }
}
...
-(void)keyBoardHiden:(NSNotification*)noti{
    ...
    if (keyBoardTap==NO) {
        ...
        if ([self isTranslucent]) {//update by qzlei
...
- (void)addToSuperView:(UIView *)superView{
    ...
    if ([self isTranslucent]) {
        frame = CGRectMake(0,s_h-kTabBarHeight-0.5,kScreenWidth, s_h+0.5);
    } else {
        frame = CGRectMake(0,s_h-kTabBarHeight-0.5-kNavBarHeight,kScreenWidth, s_h+0.5);
    }
...
//判断导航控制器的导航栏是否透明，YES表示透明，NO表示不透明
- (BOOL)isTranslucent {//update by qzlei
    UIViewController *vc = (UIViewController*)self.delegate;
    if (vc.navigationController == nil) {
        return YES;
    }
    
    return vc.navigationController.navigationBar.translucent;
}
